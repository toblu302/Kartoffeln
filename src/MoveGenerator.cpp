...
void MoveGenerator::handle_piece(Board& board, vector<Move> &moves, PIECE piece, uint64_t (MoveGenerator::*get_moves)(const Board& board, uint8_t position) ) {
    Move mv;
    mv.color = board.side;
    ...
    uint64_t pieces = getPiecesOfColor(board, board.side, piece);
...
uint64_t MoveGenerator::getPiecesOfColor(const Board& board, const COLOR& color, const PIECE& piece) {
    return board.color[color] & board.pieces[piece];
}

uint64_t MoveGenerator::getWhitePawnAttackMoves(const Board &board, uint8_t position) {
...
uint64_t MoveGenerator::getWhitePawnMoves(const Board &board, uint8_t position) {
    uint64_t pawn = (uint64_t(1) << position);
...
uint64_t MoveGenerator::getBlackPawnAttackMoves(const Board &board, uint8_t position) {
    uint64_t pawn = (uint64_t(1) << position);
...
uint64_t MoveGenerator::getBlackPawnMoves(const Board &board, uint8_t position) {
    uint64_t pawn = (uint64_t(1) << position);
    uint64_t bitwiseMove = (pawn >> 8) & ~(board.color[BLACK] | board.color[WHITE]); //moving forward 1 step
...
uint64_t MoveGenerator::getKnightMoves(const Board& board, uint8_t knight_position) {
    return KNIGHT_MOVES[ knight_position ] & ~board.color[board.side];
}

uint64_t MoveGenerator::getKingMoves(const Board& board, uint8_t king_position) {
    uint64_t bitMoves = KING_MOVES[ king_position ] & ~board.color[board.side];
    bitMoves |= getKingCastles(board, king_position);
    return bitMoves;
}

uint64_t MoveGenerator::getKingCastles(const Board& board, uint8_t king_position) {
...
uint64_t MoveGenerator::getBishopMoves(const Board& board, uint8_t bishop_position) {
...
uint64_t MoveGenerator::getRookMoves(const Board& board, uint8_t rook_position) {
...
uint64_t MoveGenerator::getQueenMoves(const Board& board, uint8_t queen_position) {
    uint64_t bitMoves = getRookMoves(board, queen_position);
    bitMoves |= getBishopMoves(board, queen_position);
    return bitMoves;
}

uint64_t MoveGenerator::getSlidingMovesFromOccupancy(const Board& board, uint8_t column, uint8_t occupancy) {
    uint64_t retValue = SLIDING_MOVES[column][occupancy];
    return retValue;
}

uint64_t MoveGenerator::getSlidingAlongRank(const Board& board, uint8_t piece_position, uint64_t blockers) {
...
uint64_t MoveGenerator::getSlidingAlongFile(const Board& board, uint8_t piece_position, uint64_t blockers) {
...
uint64_t MoveGenerator::getSlidingAlongDiagonalA1H8(const Board& board, uint8_t piece_position, uint64_t blockers) {
...
uint64_t MoveGenerator::getSlidingAlongDiagonalA8H1(const Board& board, uint8_t piece_position, uint64_t blockers) {
